...
class KeepLaneTest(Criterion):
    ...
    def __init__(self, actor, optional=False, terminate_on_failure=False, name="CheckKeepLane"):
        """
        Construction with sensor setup
        """
        super(KeepLaneTest, self).__init__(name, actor, 0, None, optional, terminate_on_failure=terminate_on_failure)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        ...
        if current_waypoint.is_junction:
            ...
            if self._pre_ego_waypoint.is_junction:
                ...
                if waypoint_angle >= self.MAX_ALLOWED_WAYPOINT_ANGLE \
                        and waypoint_angle <= (
                        360 - self.MAX_ALLOWED_WAYPOINT_ANGLE):  # pylint: disable=chained-comparison
...
class RunningStopTest(Criterion):
    ...
    def __init__(self, actor, name="RunningStopTest", list_stop_signs=None, terminate_on_failure=False):
        """
        """
        ...
        self.reach_giveway_point = None

        all_actors = self._world.get_actors()
        if not list_stop_signs:
            for _actor in all_actors:
                if 'traffic.stop' in _actor.type_id:
                    self._list_stop_signs.append(_actor)
        else:
            self._list_stop_signs = list_stop_signs

            for sign in self._list_stop_signs:
                # print(_actor.bounding_box.location)
                bb_center = sign[0].location
                bb_extent = sign[1]
                self.sign_waypoint = CarlaDataProvider.get_map(CarlaDataProvider.get_world()).get_waypoint(bb_center)
                self.sign_lower = self.sign_waypoint.previous(3)[0]
                self.sign_upper = self.sign_waypoint.next(3)[0]

                # self.A = carla.Vector3D(bb_center.x - bb_extent.x, bb_center.y - bb_extent.y, bb_center.z)
                # self.B = carla.Vector3D(bb_center.x + bb_extent.x, bb_center.y - bb_extent.y, bb_center.z)
                # self.C = carla.Vector3D(bb_center.x + bb_extent.x, bb_center.y + bb_extent.y, bb_center.z)
                # self.D = carla.Vector3D(bb_center.x - bb_extent.x, bb_center.y + bb_extent.y, bb_center.z)
                # self._world.debug.draw_line(self.A, self.B, life_time=60)
                # self._world.debug.draw_line(self.B, self.C, life_time=60)
                # self._world.debug.draw_line(self.C, self.D, life_time=60)
                # self._world.debug.draw_line(self.D, self.A, life_time=60)
                # self._world.debug.draw_string(A, 'A', draw_shadow=False,
                #                               color=carla.Color(r=0, g=255, b=0), life_time=120,
                #                               persistent_lines=True)
                # self._world.debug.draw_string(B, 'B', draw_shadow=False,
                #                               color=carla.Color(r=0, g=255, b=0), life_time=120,
                #                               persistent_lines=True)
                # self._world.debug.draw_string(C, 'C', draw_shadow=False,
                #                               color=carla.Color(r=0, g=255, b=0), life_time=120,
                #                               persistent_lines=True)
                # self._world.debug.draw_string(D, 'D', draw_shadow=False,
                #                               color=carla.Color(r=0, g=255, b=0), life_time=120,
                #                               persistent_lines=True)
    ...
    def is_actor_affected_by_stop(self, actor, stop, multi_step=20):
        ...
        """
        # print('check affected')
        affected = False
        # first we run a fast coarse test
        current_location = actor.get_location()
        # stop_location = stop.get_transform().location
        stop_location = stop[0].location
        # print('stop_location: ', stop_location, current_location)
        # print('distance to stop sign: ', stop_location.distance(current_location), self.PROXIMITY_THRESHOLD)
        if stop_location.distance(current_location) > self.PROXIMITY_THRESHOLD:
            return affected

        # stop_t = stop.get_transform()
        # stop_t = carla.Transform(stop[0].location)
        # stop_t = copy.deepcopy(stop[0])
        # transformed_tv = stop_t.transform(stop.trigger_volume.location)
        # transformed_tv = stop_t.transform(stop[0].location)
        ...
        for actor_location in list_locations:
            if self.point_inside_boundingbox(actor_location, stop[0].location, stop[1]):
                affected = True
    ...
    def update(self):
        ...
        if self.reach_time and self.leave_time:
            print(self.leave_time - self.reach_time)
            if self.leave_time - self.reach_time < 3:
                self.test_status = "FAILURE"
                self.actual_value += 1
            ...
            if current_location.distance(self.sign_lower.transform.location) <= 2:
                if not self.reach_giveway_point:
                    self.reach_giveway_point = True
                    self.reach_time = GameTime.get_time()
            elif current_location.distance(self.sign_upper.transform.location) <= 0.5:
                self.leave_time = GameTime.get_time()
        ...
        #         stop_location = self._target_stop_sign[0].location
        #         stop_extent = self._target_stop_sign[1]
        ...
        #             stop_location = self._target_stop_sign[0]
        ...
        #                     round(stop_location.location.x, 3),
        #                     round(stop_location.location.y, 3),
        #                     round(stop_location.location.z, 3)))
        #             running_stop_event.set_dict({
        #                 'id': self._target_stop_sign[2],
        #                 'x': stop_location.location.x,
        #                 'y': stop_location.location.y,
        #                 'z': stop_location.location.z})
        #
...
class DecelerateTest(Criterion):
    def __init__(self, actor, name='DecelerateTest', terminate_on_failure=False):
        super(DecelerateTest, self).__init__(name, actor, 0, terminate_on_failure=terminate_on_failure)
        self.speed_record = []
    ...
    def update(self):
        ...
        self.speed_record.append(current_speed)
        print(len(self.speed_record))

        if len(self.speed_record) > 200:
            speed_record_temp = self.speed_record[50:]
            avg_speed_1 = sum(speed_record_temp[:len(speed_record_temp) // 2]) / (len(speed_record_temp) // 2)
            avg_speed_2 = sum(speed_record_temp[len(speed_record_temp) // 2:]) / (len(speed_record_temp) // 2)
            print(avg_speed_1, avg_speed_2)
            if avg_speed_2 >= avg_speed_1:
                self.test_status = "FAILURE"
                self.actual_value += 1
...
class KeepClearTest(Criterion):
    def __init__(self, actor, name='KeepClearTest', terminate_on_failure=False):
        super(KeepClearTest, self).__init__(name, actor, 0, terminate_on_failure=terminate_on_failure)
        self.actor = actor
    ...
    def update(self):
        new_status = py_trees.common.Status.RUNNING
        current_location = CarlaDataProvider.get_location(self.actor)
        # print(self.actor, current_location)
        current_waypoint = CarlaDataProvider.get_map().get_waypoint(current_location)
        current_speed = CarlaDataProvider.get_velocity(self.actor)

        # print(current_speed, current_waypoint.is_junction)
        if self.previous_speed and self.previous_speed > 3 and current_speed == 0 and current_waypoint.previous(10)[
            0].is_junction:
            self.test_status = "FAILURE"
            self.actual_value += 1
        else:
...
class KeepDistanceTest(Criterion):
    def __init__(self, actor, other_actors, distance_second=3, delta=1, name='KeepDistanceTest',
                 terminate_on_failure=False):
        super(KeepDistanceTest, self).__init__(name, actor, 0, terminate_on_failure=terminate_on_failure)
        self.actor = actor
        self.other_actors = other_actors
        self.distance_second = distance_second
        self.delta = delta
    ...
    def update(self):
        ...
        lane_waypoints = current_waypoint.next_until_lane_end(1)
        vehicle_distances = []

        for actor in self.other_actors:
            if 'vehicle' in actor.type_id.lower():
                actor_waypoint = CarlaDataProvider.get_map().get_waypoint(CarlaDataProvider.get_location(actor))
                for waypoint in lane_waypoints:
                    if waypoint.transform.location.distance(
                            actor_waypoint.transform.location) < 3 and actor_waypoint.lane_id == current_waypoint.lane_id:
                        vehicle_distances.append(current_location.distance(actor_waypoint.transform.location))
                        break

        # print('vehicle_distances: ', vehicle_distances, current_speed)

        if vehicle_distances:
            closest_vehicle_distance = min(vehicle_distances)
            if closest_vehicle_distance < current_speed * (self.distance_second - self.delta):
                self.test_status = "FAILURE"
                self.actual_value += 1
...
class NoTurnTest(Criterion):
    def __init__(self, actor, turn_location, name='NoTurnTest', terminate_on_failure=False):
        super(NoTurnTest, self).__init__(name, actor, 0, terminate_on_failure=terminate_on_failure)
        self.turn_location = turn_location
    ...
    def update(self):
        ...
        current_location = CarlaDataProvider.get_location(self.actor)
        if current_location.distance(self.turn_location) <= 5:
            self.test_status = "FAILURE"
            self.actual_value += 1
...
class GiveWayTest(Criterion):
    def __init__(self, actor, give_way_location, name='GiveWayTest', terminate_on_failure=False):
        super(GiveWayTest, self).__init__(name, actor, 0, terminate_on_failure=terminate_on_failure)
        self._given_way_location_low = give_way_location
        self._given_way_location_high = give_way_location.next(8)[0]
        self.reach_giveway_point = False
        self.reach_time = None
        self.leave_time = None
    ...
    def update(self):
        ...
        if self.reach_time and self.leave_time:
            ...
            if current_location.distance(self._given_way_location_low.transform.location) <= 2:
                if not self.reach_giveway_point:
                    self.reach_giveway_point = True
                    self.reach_time = GameTime.get_time()
            elif current_location.distance(self._given_way_location_high.transform.location) <= 0.5:
                self.leave_time = GameTime.get_time()
                # else:
                #     current_speed = CarlaDataProvider.get_velocity(self.actor)
                #     if current_speed >= 6:
                #         self.leave_time = GameTime.get_time()
            # else:
            #     if self.reach_time and not self.leave_time:
            #         self.leave_time = GameTime.get_time()
