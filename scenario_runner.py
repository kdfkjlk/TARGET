...
class ScenarioRunner(object):
            ...
            while ego_vehicle_missing:
                ...
                for ego_vehicle in ego_vehicles:
                    ...
                    for carla_vehicle in carla_vehicles:
                        if carla_vehicle.attributes['role_name'] == ego_vehicle.rolename or carla_vehicle.attributes['role_name'] == "ego_vehicle":
    ...
    def _analyze_scenario(self, config):
        ...
        filename = None
        if self._args.file:
            filename = self._args.file + current_time + ".txt"
        ...
        if self._args.reloadWorld:
            ...
            if self._args.waitForEgo:
                while not ego_vehicle_found and not self._shutdown_requested:
                    ...
                    actors = self.client.get_world().get_actors()
                    vehicles = []
                    for actor in actors:
                    ...
                    for ego_vehicle in ego_vehicles:
                        ...
                        for vehicle in vehicles:
                            ...
                            if vehicle.attributes['role_name'] == ego_vehicle.rolename or vehicle.attributes['role_name'] == 'ego_vehicle':
                                ego_vehicle_found = True
                                break
                        if not ego_vehicle_found:
    ...
    def _load_and_run_scenario(self, config, rule_scenario=None):
        """
            ...
            if self._args.openscenario:
                ...
                if not rule_scenario:
                    ...
                    scenario = scenario_class(self.world,
                                              ...
                                              self._args.debug)
                    if config.agent and config.destination:
                        waypoint_list = []
                        waypoint_list.append(config.trigger_points[0].location)
                        for point in config.destination:
                            waypoint_list.append(point.location)
                        config.trajectory = waypoint_list
                        gps_route, route = interpolate_trajectory(self.world, config.trajectory)
                        config.route = route
                        config.gps_route = gps_route
                        config.agent.set_global_plan(config.gps_route, config.route)
    ...
    def run_parsed_scenario(self, config, scenario):
        result = self._load_and_run_parsed_scenario(config, scenario)
        print("No more scenarios .... Exiting")
        test_results = self.manager.output_test_result()
        return test_results

    def _load_and_run_parsed_scenario(self, config, scenario):
        """
        Load and run the scenario given by config
        """
        result = False
...
def main():
    ...
    parser.add_argument(
        '--scenario',  default='Night_2', help='Name of the scenario to be executed. Use the preposition \'group:\' to run all scenarios of one class, e.g. ControlLoss or FollowLeadingVehicle')
    ...
    parser.add_argument(
        '--agent', default='/home/yao/Documents/carla/carla-expert/team_code/expert_agent/auto/auto_expert.py', help="Agent used to execute the scenario. Currently only compatible with route-based scenarios.")
    parser.add_argument('--agentConfig', default='/home/yao/Documents/carla/carla-expert/team_code/config/agent_config/mmfn.yaml', type=str, help="Path to Agent's configuration file")
    ...
    parser.add_argument('--reloadWorld', default=True, action="store_true",
    ...
    parser.add_argument('--waitForEgo', default=False, action="store_true", help='Connect the scenario to an existing ego vehicle')
