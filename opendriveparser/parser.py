...
def parse_opendrive(rootNode):
    ...
    for road in rootNode.findall("road"):
        ...
        if road.find("objects") is not None:
            for object in road.find("objects").findall("object"):
                newObject = {'id': object.attrib['id']}
                if 'crosswalk' in object.attrib['name'].lower():
                    newObject['name'] = 'crosswalk'
                    if not newOpenDrive.has_crosswalk:
                        newOpenDrive.has_crosswalk = True

                elif 'speed_' in object.attrib['name'].lower() or 'speedlimit' in object.attrib['name'].lower():
                    newObject['name'] = 'speed_sign'
                    try:
                        newObject['speed_limit'] = int(object.attrib['name'].split('_')[1])
                    except:
                        newObject['speed_limit'] = int(object.attrib['name'][10:12])
                    if not newOpenDrive.has_speed_sign:
                        newOpenDrive.has_speed_sign = True
                    # newObject['validity_lanes'] = []
                    # validity_lanes = []

                else:
                    newObject['name'] = object.attrib['name']

                validity_lanes = []
                if newObject['name'] == 'crosswalk':
                    for lane in newRoad.lanes.laneSections[0].allLanes:
                        if lane.type == 'driving':
                            validity_lanes.append(lane.id)
                else:
                    if object.attrib['orientation'] == '-':
                        for lane in newRoad.lanes.laneSections[0].rightLanes:
                            if lane.type == 'driving':
                                validity_lanes.append(lane.id)
                    else:
                        for lane in newRoad.lanes.laneSections[0].leftLanes:
                            if lane.type == 'driving':
                                validity_lanes.append(lane.id)
                newObject['validity_lanes'] = validity_lanes
                newRoad._objects.append(newObject)

        # Signals
        if road.find("signals") is not None:
            for signal in road.find("signals").findall("signal"):
                if signal.attrib['id'] not in newOpenDrive._signals:
                    newOpenDrive._signals[signal.attrib['id']] = signal.attrib['name']
                # print(signal.attrib['name'], signal.attrib['id'])
            for signal_ref in road.find("signals").findall("signalReference"):
                validity = signal_ref.find("validity")
                validity_lanes = list(range(int(validity.attrib['fromLane']), int(validity.attrib['toLane'])+1))
                if signal_ref.attrib['id'] in newOpenDrive._signals:
                    newRoad._signals.append({'id': signal_ref.attrib['id'],
                                         'name': newOpenDrive._signals[signal_ref.attrib['id']],
    ...
    for road in newOpenDrive.roads:
        if len(road.signals) != 0:
            for signal in road.signals:
                if 'name' not in signal:
                    signal['name'] = newOpenDrive._signals[signal['id']]
