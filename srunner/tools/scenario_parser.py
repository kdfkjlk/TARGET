    ...
    def parse_scenario_configuration(scenario_name, config_file_name):
        ...
        for file_name in list_of_config_files:
            ...
            for scenario in tree.iter("scenario"):
                ...
                for destination_node in scenario.iter('destination'):
                    pos_x = float(destination_node.attrib.get('x', 0))
                    pos_y = float(destination_node.attrib.get('y', 0))
                    pos_z = float(destination_node.attrib.get('z', 0))
                    yaw = float(destination_node.attrib.get('yaw', 0))
                    transform = carla.Transform(carla.Location(x=pos_x, y=pos_y, z=pos_z), carla.Rotation(yaw=yaw))

                    new_config.destination.append(transform)
